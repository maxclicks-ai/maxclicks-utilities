...
export class Normalizer<Value> {
  ...
  normalize<V>(
    ...
    IfAny<
      V,
      Value,
      Exclude<V, null> extends DeepReadonly<Value> ? (null extends Value ? V | null : Exclude<V, null>) : Value
    >
  > {
    ...
    try {
      ...
      return new Normalizer.Normalized({
        warningMessage: warn.accumulatedMessage,
  ...
  normalizeIfExists<V>(
    ...
    IfAny<
      V,
      Value | undefined,
      Exclude<V, null | undefined> extends DeepReadonly<Value>
        ? (null extends Value ? V | null : Exclude<V, null>) | (undefined extends V ? undefined : never)
        : Value | undefined
    >
  > {
    const warn = Warn.create()
  ...
  export class Async<Value> {
    ...
    > {
      ...
      try {
        ...
        if (value === undefined) throw new Error('Undefined.')
        const parsedValue = await this.parseOrThrowAsync(value, warn, abortSignal)
        if (abortSignal?.aborted) throw new Error('Aborted.')
    ...
    static combine<T extends { readonly [K in string]: any } | readonly any[]>(
      ...
      const normalizedItemsWithRedefinedKeys = (
        Array.isArray(normalizedItems)
          ...
          : arrayHelpers.toDictionary(
              objectHelpers.entries(normalizedItems),
              ([key, value]) => (labelByKey?.[key] || key) as string,
              ([key, value]) => value
            )
